...
class CoinPayments extends Model
{
     ...
     * @param $short, If short == true (the default), the output won't include the currency names and confirms
     * needed to save bandwidth.
     * @return mixed
     */
     ...
     * @param $currency2. The cryptocurrency of the transaction. currency1 and currency2 can be the same if you don't
     * want any exchange rate conversion.
     * @param $address. Optionally set the payout address of the transaction. If address is empty then it will follow
     * your payout settings for that coin.
     * @param $ipn_url. Optionally set an IPN handler to receive notices about this transaction. If ipn_url is empty
     * then it will use the default IPN URL in your account.
     * @param $buyer_email. Optionally (recommended) set the buyer's email so they can automatically claim refunds if
     * there is an issue with their payment.
     * @return mixed
     */
    public function createTransactionSimple(
        $amount,
        $currency1,
        $currency2,
        $address = '',
     ...
     * @param $ipn_url. Optionally set an IPN handler to receive notices about this transaction.
     ...
     * @param $auto_confirm. If auto_confirm is TRUE, then the withdrawal will be performed without an email
     * confirmation.
     * @param $ipn_url. Optionally set an IPN handler to receive notices about this transaction. If ipn_url is empty
     * then it will use the default IPN URL in your account.
     * @return mixed
